...
def fetch_last_day_price_info(conn):
    ...
    if yesterday.weekday() == 6:  # Sunday
        ...
        yesterday = cursor.fetchone()[0]  # Use the last available date if no data for yesterday
...
def run(symbol, tick, indicators, market_is_open=True):
    ...
    if cond1 and cond2 and cond3 and cond4:
        ...
        log_signal(f"åtkt modified tsl: Conditions not met for {symbol}: RSI={rsi_val}, SMA={sma200}, Close={close_today}, yesterday high={last_day['high']}, time={tick['timestamp']}")
