...
tick_data AS (
    SELECT
        ...
        (date_trunc('hour', timestamp) + interval '1 hour') AS interval_close_time,
        ROW_NUMBER() OVER (
            PARTITION BY date_trunc('hour', timestamp) + interval '1 hour'
...
INSERT INTO ohlc_1h (symbol_code, candle_close_time, open_price, high_price, low_price, close_price, volume)
SELECT
