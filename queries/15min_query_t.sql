...
tick_data AS (
    SELECT
        ...
        md.timestamp,
        (date_trunc('minute', md.timestamp) - (extract(minute from md.timestamp)::int % 15) * interval '1 minute' + interval '15 minutes') AS interval_close_time,
        ROW_NUMBER() OVER (
            PARTITION BY md.symbol_code, (date_trunc('minute', md.timestamp) - (extract(minute from md.timestamp)::int % 15) * interval '1 minute' + interval '15 minutes')
...
INSERT INTO ohlc_15min (symbol_code, candle_close_time, open_price, high_price, low_price, close_price, volume)
SELECT
    t1.symbol_code,
