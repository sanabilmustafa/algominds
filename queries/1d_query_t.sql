--- daily data but without open_price 
...
tick_data AS (
  SELECT
      ...
      md.open_price,
      md.timestamp,
      (date_trunc('day', timestamp)) AS interval_close_time,
      ROW_NUMBER() OVER (
          PARTITION BY md.symbol_code, date_trunc('day', timestamp) + interval '1 day'
      ...
      ROW_NUMBER() OVER (
		  ...
		  ORDER BY 
		    CASE 
		      WHEN md.last_trade_price IS NOT NULL AND md.last_trade_price > 0 THEN md.timestamp
		      ELSE NULL
		    END ASC
			) AS rn_open_real
  FROM market_data md
...
INSERT INTO ohlc_1d (symbol_code, candle_close_time, open_price, high_price, low_price, close_price, volume)
SELECT
  t1.symbol_code,
  t1.interval_close_time AS candle_close_time,

  -- ✅ open_price from first tick of the day
  (SELECT open_price
   FROM tick_data t_open
   WHERE t_open.symbol_code = t1.symbol_code
     AND t_open.interval_close_time = t1.interval_close_time
     AND t_open.rn_close = 1
